...
class Database {
	...
	public function db_update($table_name,$db_columns,$db_conditions,$isOr=false)
	{
        ...
        if (mysqli_connect_errno()) {       ////throw an error if there is a connection problem
            log_error('DB: ' . $q . ' ERROR INSERTING. BAD QUERY.'. PHP_EOL);

            return false; //mysqli_connect_error());
        } else {   //transaction successful
            $db_conn->query($q); //or $this->db_error($result);
            $result =$db_conn->affected_rows;
            $db_conn->close();
            log_error('DB: ' . $q . PHP_EOL . ' result:' . $result);
    ...
    public function db_create($table_name,$db_columns)
    {
        ...
        if (mysqli_connect_errno()) {       ////throw an error if there is a connection problem
            ...
            $db_conn->close();
            log_error('DB: ' . $q . PHP_EOL . ' result:' . $id);
    ...
    public function db_retrieve($table_name,$db_columns,$db_conditions='',$db_extra=null,$isOr=false)
    {
        ...
        log_error('Checking for key in cache: ' . $key);
        if(!is_null($data))
        {
            log_error('Cache found: ' . $key);
        ...
        if (mysqli_connect_errno()) {
            ...
            log_error('DB: ' . $q . PHP_EOL .  print_r($result_set,true). PHP_EOL);
    ...
    public function db_query($q,$db_database=DATABASE_NAME)
    {
        ...
        if (mysqli_connect_errno()) {
            ...
            if(stripos($q,'update') > -1) {
                $result = $db_conn->query($q); //or $this->db_error($result);
                log_error('DB: ' . $q . PHP_EOL .  print_r($result,true). PHP_EOL);
                return $db_conn->affected_rows;
            } elseif(strpos($q,'SQL_CALC_FOUND_ROWS'))
            {
                ...
                if ($result) {
                    do {
                        ...
                        if(isset($result_set[0]['row_count']))
                        {
                            ...
                            //log_error('row_count in multiquery found: row_count=' . $row_count);

                        } else
                        {
                            if(!empty($result_set)) {
                                log_error('main result set found: ' . print_r($result_set,true));
                ...
                log_error('DB: ' . $q . PHP_EOL .  print_r($result,true));
            ...
            //log_error('DB: ' . $q . PHP_EOL .  print_r($result_set,true));
            //log_error('DB: ' . $q );
