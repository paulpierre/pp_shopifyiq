...
var CRAWLER_TIMEOUT = 1000 * 60 * 30; //5 minute time out for scraping the entire catalog
var ENABLE_LOGGING;
var SERVER_POST_URL;
switch (MODE)
{
...
var store_data = json;
error_log("Scraping site: " + json[0].domain);
...
/* =========================
 ...
 store_start_page = json[0].start_page,
 store_end_page = json[0].end_page;
...
function parse_results(store_data)
{
    ...
    if(page.injectJs('shopifyiq_crawler.js')) console.log("SUCCESS: injectJs: shopifyiq_crawler.js");
    ...
    waitFor(function(product_list) {
        ...
        return page.evaluate(function() {
            /*
            console.log(
                "\nis_crawling_product_details:"+is_crawling_product_details +
                "\nis_crawling_product: "+ is_crawling_product +
                "\nis_crawling_page: " + is_crawling_page +
                "\nproduct_list: " + product_list
            );*/
            return (!is_crawling_product_details && !is_crawling_product && !is_crawling_page && typeof product_list !== undefined && product_list.length > 1);
        });
        ...
        product_list =  page.evaluate(function() {
            console.log("evaluate product_list: " + product_list);
...
function store_results(data)
{
    ...
    p.open( SERVER_POST_URL, "POST", "crawl_tstart=" + crawl_tstart  + "&store=" +  encodeURIComponent(_json)+ "&products=" + encodeURIComponent(data), function(status) {
        if (status !== 'success') {
