...
if [ "${_HOSTNAME}" == "##########" ] || [ "${_HOSTNAME}" == "##########" ]; then
    ...
    API_PATH="##########/shopifyiq/shopifyiq/src/api"
...
SHOPIFY_DEBUG_STORE_ID=29 #Store to manually recheck
...
if [ "${SHOPIFY_DEBUG_STORE}" == 1 ]; then
    sql_query="SELECT store_id, store_domain, store_url, store_target_click_url, store_target_img, store_target_node, store_target_price, store_target_name, store_target_details, store_target_page_count, store_details_type, store_page_count_type, store_total_page_count FROM stores WHERE store_id = ${SHOPIFY_DEBUG_STORE_ID}";
fi
...
while read -r line; do
    ...
    STORE_LIST[COUNTER]=$(echo "${STORE_ID}|${STORE_DOMAIN}|${STORE_URL}|${STORE_TARGET_CLICK_URL}|${STORE_TARGET_IMG}|${STORE_TARGET_NODE}|${STORE_TARGET_PRICE}|${STORE_TARGET_NAME}|${STORE_TARGET_DETAILS}|${STORE_TARGET_PAGE_COUNT}|${STORE_DETAILS_TYPE}|${STORE_PAGE_COUNT_TYPE}|${STORE_TOTAL_PAGE_COUNT}");
...
while [ ${COUNTER} -lt ${thread_count} ]; do
        ...
        while [ ${CTR} -le ${STORE_TOTAL_PAGE_COUNT} ]; do

            printf "\n\nSCRAPING WEBSITE $STORE_DOMAIN PAGE $CTR of $STORE_TOTAL_PAGE_COUNT\n\n"

            let STORE_START_PAGE=CTR
            let STORE_END_PAGE=CTR

            #If the starting page is 1, lets make sure the end page is 1 as well.
            #if [ ${STORE_START_PAGE}==1 ]; then
            #    let STORE_END_PAGE=STORE_START_PAGE
            #else
            #   let STORE_END_PAGE=CTR+1
            #fi



            JSON_STR="["
            #echo "Order ID: $ORDER_ID Fulfillment ID: $FULFILLMENT_ID Tracking #: $TRACKING_NUMBER"
            JSON_STR+="{\"start_page\":\"$STORE_START_PAGE\",\"end_page\":\"$STORE_END_PAGE\",\"id\":\"$STORE_ID\",\"domain\":\"$STORE_DOMAIN\",\"url\":\"$STORE_URL\",\"t_url\":\"$STORE_TARGET_CLICK_URL\",\"t_img\":\"$STORE_TARGET_IMG\",\"t_node\":\"$STORE_TARGET_NODE\",\"t_price\":\"$STORE_TARGET_PRICE\",\"t_name\":\"$STORE_TARGET_NAME\",\"t_details\":\"$STORE_TARGET_DETAILS\",\"t_page_count\":\"$STORE_TARGET_PAGE_COUNT\",\"details_type\":\"$STORE_DETAILS_TYPE\",\"count_type\":\"$STORE_PAGE_COUNT_TYPE\"},"
            ...
            if [ "${PROXIES_ENABLED}" -eq "1" ]; then
                ...
                ./bin/${PHANTOMJS_BIN} $PHANTOMJS_DEBUG --load-images=no --ignore-ssl-errors=true --web-security=false --disk-cache=true scripts/${PHANTOM_SCRIPT} "$JSON"
                #exit 0
