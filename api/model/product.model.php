...
class Product extends Database {
    ...
    public $FK_PREFIX_ARRAY      = Array('store_','variant_','crawl_','owner_');
    ...
    public function __construct($o = null)
    {
         ...
         *  -----------------------------------------
         */
        if($this->id !== null) $this->get_product_variants();
    ...
    public function save($o = null)
    {
        ...
        if(!is_array($o) && $o instanceof Product) $o = $o->id;
    ...
    public function get_product_variants()
    {
        ...
        $db_conditions = Array();
        $db_conditions[self::TABLE_PREFIX . 'id'] = $o_id;
        $result = $this->db_retrieve(self::TABLE_NAME,Array('variant_id'),$db_conditions,null,false);
        if(empty($result))
            ...
            foreach($result as $o)
            {
                ...
                $this->product_variants[] = new Variant($product_id);
            }
        }
        return $result;
    }
     ...
     *  --------------------------------
     */
    public function fetch_product_by_shopify_product_id($product_id,$store_id)
    {
        $db_conditions = Array(
            'product_shopify_id'=>$product_id,
            'store_id'=>$store_id
        );
        $db_columns = Array('product_id');
        $result = $this->db_retrieve(self::TABLE_NAME,$db_columns,$db_conditions,null,false);
        if(empty($result)) return false;
        else return new Product(intval($result[0]['product_id']));
    }
